...
def main():
    ...
    text = input("Enter a command (e.g., diagonal forward right): ")
    prompt = (
        f"Convert the following user instruction into a sequence of robot commands "
        f"(forward/backward/left/right/diagonal_forward_right) "
        f"in JSON format like [\"diagonal_forward_right\"]: {text}"
    )
